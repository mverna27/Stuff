...
# Clone of `playerctl`. Sends 'Previous' to the first argument's media player.
function pctl {
...
for player in $(playerctl -l); do
    ...
    if [[ "$player" == *"spotifyd"* ]]; then
        # and playing, 'Previous' it.
        if [[ "$spt_status" == *"Playing"* ]]; then
            pctl $player "Previous"
            exit 0
        fi
    else
        # Else, if the status of the player is `Playing`, 'Previous' it and exit
